...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # used to associate the socket with IPv4 family address of the server
        # and port number is the specific default port of the serve
        proxy_socket.bind((proxy_host, proxy_port))
        # The function is listening for unaccepted connections
        ...
        while True:
            try:
                # To accept() func is blocks execution
                # and waits for an incoming connection.
                # When a client connects,
                # it returns a new socket object representing the connection and
                # a tuple holding the address of the client.
                # The func is major for connection between client and server
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int],
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # to read the client's reply and then calculate the request to send it back to the client
            ...
            try:
                ...
                # sendall() to send response after calculate to client
                client_socket.sendall(response)
                # close() is take care on remove the connections
                client_socket.close()
