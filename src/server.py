...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # and port number is the specific default port of the server
        ...
        while True:
            try:
                # To accept() func is blocks execution and waits for an incoming connection.
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # to read the client's reply
            # and then calculate the request to send it back to the client
            ...
            try:
                ...
                #  to send response after calculate to client
